$(document).ready(function() {
  ...
  $.getJSON('/api/books', function(data) {
    ...
    $('#display').on('click', 'li.bookItem', function() {
      ...
      $.getJSON('/api/books/' + itemsRaw[this.id]._id, function(data) {
        ...
        comments.push('<br><form action="" method="post" id="newCommentForm"><input style="width:300px" type="text" class="form-control" id="commentToAdd" name="comment" placeholder="New Comment"></form>');
  ...
  $('#deleteAllBooks').click(function() {
    $.ajax({
      ...
      success: function(data) {
        //update list
        $('#detailComments').html('complete delete successful');
      }
    });
  });
});
